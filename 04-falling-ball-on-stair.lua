...
local size = { width = 960*scale, height = 640*scale }
...
-- local ball = Util:character({ file = 'images/kaw-logo.png', width = 600, height = 600, scale = 0.1*scale, name = "Football"})
...
steps = 4
step_width = size.width/steps
for i=1,steps do
  ground_fixture  = ground_body:addRect(-size.width/2+(step_width*(i-1)), -size.height/2,-size.width/2+(step_width*(i)),(20*i)-size.height/2)
  -- ground_fixture:setFilter ( 0x02 )
  -- ground_fixture:setLoc(0,-200)  
end

wall_fixture = ground_body:addRect(-size.width/2,-size.height/2,5-size.width/2,size.height/2)
...
function onMove(x,y)
  ...
  if pick then
    ...
    pick.fixture:getBody():setTransform ( mouseX, mouseY )
  end
end
